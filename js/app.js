...
const updateMessage = () => {
  if (winner === false && loser === true && gameOver === true) {
    ...
    messageEl.innerText = `Congrats you got all the cheese. Click the reset button to start a new game.`;
    winner = false;
  } else if (winner === false && loser === false) {
    messageEl.innerText = `Collect all the cheese to win by clicking the squares and avoid the cat.`;
  }
};
...
const checkIfWinner = () => {
  if (!winner) {
    if (
      mouseTracker.every(
        (num) => winningCombo.includes(num) && mouseTracker.length === 9
      )
    ) {
      winner = true;
      loser = false;
